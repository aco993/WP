$(document).ready(function(){
    ...
    $('#radni_prostor').on('click', '#btnAddKategoriju', function(){
    	...
    	var check_ime = /^[A-Z]{1}[a-z]{1,14}([_]{1}[A-Z]{1}[a-z]{1,14})*$/;
  ...
  //sacuvaj novu prodavnicu
    ...
    $('#radni_prostor').on('click', '#btnDodajProizvod', function(){
    	...
    	$("#radni_prostor").append("<table border=2><tr><td>Sifra:</td><td><input type=\"text\" id=\"sifra\" maxlength=10></td></tr>"+
    			...
    			"<tr><td>Dimenzije: </td><td><input type=\"text\" id=\"dimenzije\" maxlength=4></select></tr>"+
    			"<tr><td>Tezina: </td><td><input type=\"text\" id=\"tezina\" maxlength=4></td></tr>"+
    			"<tr><td>Zemlja Proizvodnje: </td><td><select id=\"zemljaProizvodnje\"></td></tr>"+
    			"<tr><td>Naziv proizvodjaca: </td><td><input type=\"text\" id=\"nazivProizvodjaca\" maxlength=15></td></tr>"+
    			"<tr><td>Jedinicna cena: </td><td><input type=\"text\" id=\"jedinicnaCena\" maxlength=4></td></tr>"+
    			...
    			"<tr><td>Kolicina u magacinu: </td><td><input type=\"text\" id=\"kolicinaUMagacinu\" maxlength=4></td></tr>"+
    			...
    			"<tr><td></td><td><button id=\"btnAddProizvod\">Add Proizvod!</button></td></tr></table>");
  ...
  //obrisi proizvod
  	          ...
  	          function(dataa,status){
					  alert("Uspesno preuzet proizvod!!!"+dataa.sifra);
				    	...
				    	$("#radni_prostor").append("<table border=2><tr><td>Sifra:</td><td><input type=\"text\" id=\"sifra\" maxlength=10></td></tr>"+
    							...
    							"<tr><td></td><td><button id=\"btnReplaceProizvod\">Replace Proizvod!</button></td></tr></table>");
    ...
    //dodaj akciju		
    $('#radni_prostor').on('click', '#btnPostaviAkciju', function(){		
    	var proizvodS=$(this).parent().parent().attr("id");		
    	var prodavnicaS=$(this).parent().parent().attr("id2");            		
    	$("#radni_prostor").empty();		
    	$("#radni_prostor").append("<h1>POSTAVI AKCIJU<h1>");		
    	$("#radni_prostor").append("<table border=2><tr><td>Procenat:</td><td><input type=\"text\" id=\"procenat\" maxlength=2></td></tr>"+		
    			"<tr><td>Pocetak vazenja:</td><td><input type=\"text\" id=\"pocDatum\" ></td></tr>"+		
    			"<tr><td>Kraj vazenja: </td><td><input type=\"text\" id=\"krajDatum\" ></td></tr>"+		
    "<tr><td></td><td><button id="+proizvodS+" id2="+prodavnicaS+" class=btnPotvrdiAkciju>Postavi akciju</button></td></tr></table>"); 		
    	            		
    });		
    		
    $('#radni_prostor').on('click', '.btnPotvrdiAkciju', function(){		
    			
    	var sifra=Math.floor((Math.random() * 900000) + 100000);		
    	var proizvod=$(this).attr("id");		
    	var prodavnica=$(this).attr("id2");		
    	alert(prodavnica);		
    	...
    	var check_procenat = /^[0-9]{1,2}$/;		
    	if(!check_procenat.test(procenat))		
    		greska+="Neispravno unesen procenat! :)";   		
    			
		  $.post("http://localhost:8080/Projekat/AddAkcijaServlet",		
    			  {		
    			    data:JSON.stringify({		
    			    	sifra:sifra,		
    			    	prodavnica:prodavnica,		
    ...
    $('#radni_prostor').on('click', '#btnPregledRecenzijaProdavnice', function(){		    
    	PregledRecenzijaProdavnice($(this).parent().parent().attr("id"));		
    });
    ...
    $("#radni_prostor").on('click','#btnPogledajKupovinu',function(){		
    	var sifra=$(this).parent().parent().attr("id");		
  PregledKupovine(sifra);		
  				//TODO: pregled kupovine   			
  });
...
{
	$("#radni_prostor").empty();
	$("#radni_prostor").append("<button id=\"btnDodajKategoriju\">Dodaj Novu Kategoriju!</button>");
...
function DodajKategoriju(tip)
{
	...
	if(tip=="dodavanje"){
		...
		$("#radni_prostor").append("<table border=2><tr><td>Naziv kategorije:</td><td><input type=\"text\" id=\"naziv\" maxlength=15></td>"+
				...
				//"<tr><td></td><td><input type=\"button\" id=\"btnAddKategoriju\" value=\"Add kategoriju!\"></td></tr></table>");
				"<tr><td></td><td><button id=\"btnAddKategoriju\">Add Kategoriju!</button></td></tr></table>");
	}
	else{
		...
		$("#radni_prostor").append("<table border=2><tr><td>Naziv kategorije:</td><td><input type=\"text\" id=\"naziv\" maxlength=15></td>"+
				...
				"<tr><td></td><td><button id=\"btnReplaceKategoriju\">Replace kategoriju!</button></td></tr></table>");
	}
...
{
	$("#radni_prostor").empty();
	$("#radni_prostor").append("<button id=\"btnDodajDostavljaca\" >Dodaj Novog Dostavljaca!</button>");
...
function DodajDostavljaca(tip)
{
	...
	if(tip=="dodavanje"){
		...
		$("#radni_prostor").append("<table border=2><tr><td>Sifra:</td><td><input type=\"text\" id=\"sifra\" maxlength=10></td></tr>"+
			...
			"<tr><td></td><td><button id=\"btnAddDostavljac\">Add Dostavljac!</button></td></tr></table>");
	}else{
		...
		$("#radni_prostor").append("<table border=2><tr><td>Sifra:</td><td><input type=\"text\" id=\"sifra\" maxlength=10></td></tr>"+
			...
			"<tr><td>Zemlje: </td><td id=\"zadodavanje\"></td></tr>"+
			"<tr><td>Cena: </td><td><input type=\"text\" id=\"cena\" maxlength=4></td></tr>"+
			"<tr><td></td><td><button id=\"btnReplaceDostavljac\">Replace Dostavljac!</button></td></tr></table>");
		$("#sifra").val(tip.sifra);
    	$("#sifra").prop("readonly",true);
    	$("#naziv").val(tip.naziv);
    	$("#opis").val(tip.opis);
...
function PregledProdavnica(){
	$("#radni_prostor").empty();
	$("#radni_prostor").append("<button id=\"btnDodajProdavnicu\" >Dodaj Novu Prodavnicu!</button>");
	$("#radni_prostor").append("<table border=2 id=\"tblProdavnice\"><tr><th>Sifra</th><th>Naziv</th><th>Adresa</th><th>Drzava</th>" +
			"<th>Telefon</th><th>Email</th><th>Odgovorni prodavac</th><th>Ocena</th><th id=headerRecenzijeProdavnice>Recenzije</th>"+
	    	...
	    	function(data,status){ 
				    	for(var i = 0; i<data.length; i++){
				    		$("#tblProdavnice").append("<tr id="+data[i].sifra+"><td>"+data[i].sifra+"</td><td>"+data[i].naziv+"</td>" +
				    				...
				    				"<td><button id=\"btnPregledRecenzijaProdavnice\">Pregled Recenzija!</button></td>"+
	...
	$.post("DobaviLogovanogKorisnikaServlet",
			function(data,status){
				if(data.uloga=="prodavac"){
					...
					$("#btnPregledRecenzijaProdavnice").hide();
					$("#headerRecenzijeProdavnice").hide();
					$('td:nth-child(9)').hide();
...
function DodajProdavnicu(tip){
	...
	if(tip=="dodavanje"){
		...
		$("#radni_prostor").append("<table border=2><tr><td>Sifra:</td><td><input type=\"text\" id=\"sifra\" maxlength=10></td></tr>"+
				...
				"<tr><td></td><td><button id=\"btnAddProdavnica\">Add Prodavnicu!</button></td></tr></table>");
	}else{
		...
		$("#radni_prostor").append("<table border=2><tr><td>Sifra:</td><td><input type=\"text\" id=\"sifra\" maxlength=10></td></tr>"+
				...
				"<tr><td>Email: </td><td><input type=\"text\" id=\"email\" maxlength=20></td></tr>"+
				"<tr><td>Odgovorni prodavac: </td><td><select id=\"selekcijaProdavca\"></select></td></tr>"+
				"<tr><td></td><td><button id=\"btnReplaceProdavnica\">Replace Prodavnicu!</button></td></tr></table>");
...
function Proizvodi() {		
	...
	$("#radni_prostor").append("<button id=\"btnDodajProizvod\">Dodaj Novi Proizvod</button>");		
	...
	$("#radni_prostor").append("<table border=2 id=\"tblProizvodi\"><tr><th>Sifra</th><th>Naziv</th><th>Boja</th><th>Dimenzije</th>" +		
			...
			"<th>Ocena</th><th>Recenzije</th><th>Izmena</th><th>Brisanje</th><th>Akcija</th></tr> </table> ");		
			
	$.post("http://localhost:8080/Projekat/DobaviProizvodeServlet",		
	    	function(data,status){ 		
				    	for(var i = 0; i<data.length; i++){		
				    		$("#tblProizvodi").append("<tr id="+data[i].sifra+" id2="+data[i].prodavnica+"><td>"+data[i].sifra+"</td><td>"+data[i].naziv+"</td>" +		
				    				...
				    				"<td><button id=\"btnIzmeniProizvod\">Izmeni Proizvod!</button></td><td><button id=\"btnObrisiProizvod\">Obrisi proizvod!</button></td>"+		
				    				"<td><button id=\"btnPostaviAkciju\" >Postavi akciju</button></td></tr>");		
				    	}		
			}					
	);  			
...
/////////////////////////////ZA RECENZIJE PRODAVNICA//////////////////////////////////		
function PregledRecenzijaProdavnice(tip){		
	$("#radni_prostor").empty();		
	$("#radni_prostor").append("<table border=2 id=\"tblZaRecenzijeProdavnica\"><tr><th>Sifra</th><th>Kupac</th><th>Naziv Prodavnice</th>" +		
			"<th>Datum</th><th>Komentar</th><th>Ocena</th><th>Obrisi</th></tr> </table> ");		
	...
	$.post("http://localhost:8080/Projekat/DobaviRecenzijeProdavnica",		
	    	...
	    	function(data,status){ 		
				    	for(var i = 0; i<data.length; i++){		
				    		$("#tblZaRecenzijeProdavnica").append("<tr id="+data[i].sifra+"><td>"+data[i].sifra+"</td><td>"+data[i].kupac+"</td><td>"+data[i].nazivProdavnice+"</td>" +		
...
function PregledKupovine() {		
alert('moram implementirati')			
$("#radni_prostor").empty();		
}